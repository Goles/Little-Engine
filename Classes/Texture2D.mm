...
@implementation Texture2D (privateMethods)
...
- (id) initWithImagePath:(NSString*)path sizeToFit:(BOOL)sizeToFit pixelFormat:(Texture2DPixelFormat)pixelFormat filter:(GLuint)inFilter
{
	...
	if(self == nil)
		NSLog(@"Failed loading image at path \"%@\"", path);
...
- (id) initWithImage:(UIImage *)uiImage filter:(GLenum) filter
{
	...
	if(!self)
		NSLog(@"Unable to init Texture2D from image");
		...
		if(CGImageGetColorSpace(image)) {
			if(CGColorSpaceGetModel(CGImageGetColorSpace(image)) == kCGColorSpaceModelMonochrome) {
				if(hasAlpha) {
					...
					NSLog(@"Unoptimal image pixel format for image at path \"%@\"", path);
#endif
				}
				else {
				...
				pixelFormat = kTexture2DPixelFormat_RGBA5551;
					...
					pixelFormat = kTexture2DPixelFormat_RGBA8888;
						...
						NSLog(@"Unoptimal image pixel format for image at path \"%s\"", path);
#endif
					}
				}
			}		
...
#if __DEBUG__
		NSLog(@"Image at %ix%i pixels is too big to fit in texture", width, height);
#endif
		width /= 2;
		height /= 2;
...
- (void) drawAtPoint:(CGPoint)point depth:(CGFloat)depth
{
	...
	//Strided Array format is <x, y> + <s,t>
    GLfloat				position[] = {
		-width + point.x,  -height + point.y,	0, 1,
		width + point.x,  -height + point.y,	1, 1,
		-width + point.x,	height + point.y,	0, 0,
		width + point.x,	height + point.y,	1, 0,
	};	
	
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(position), position, GL_DYNAMIC_DRAW);
	glEnableVertexAttribArray(ATTRIB_POSITION);
    glVertexAttribPointer(ATTRIB_POSITION, 2, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * 4, ( const GLvoid * ) ( 0 * sizeof(GLfloat)));
    glEnableVertexAttribArray(ATTRIB_TEXCOORD);
	glVertexAttribPointer(ATTRIB_TEXCOORD, 2, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * 4, ( const GLvoid * ) ( 2 * sizeof(GLfloat)));
